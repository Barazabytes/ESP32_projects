...
static esp_err_t dht_init() {
    ...
    // drive idle level 1
    hal_gpio_write(dht_rmt.gpio, 1);
    hal_gpio_pullup_dis(dht_rmt.gpio);
    hal_gpio_pulldown_dis(dht_rmt.gpio);
    hal_gpio_set_direction(dht_rmt.gpio, HAL_GPIO_MODE_INPUT_OUTPUT_OD);
    hal_gpio_set_intr_type(dht_rmt.gpio, HAL_GPIO_INTR_DISABLE);

    return ESP_OK;
}

static esp_err_t _dht_read(uint8_t wakeup_ms, uint8_t *data) {
    if (dht_init(sensor.gpio_pin) != ESP_OK) {
        return ESP_ERR_INVALID_RESPONSE;
    }

    // send start signal and arm RX channel
    TickType_t xDelay = pdMS_TO_TICKS(wakeup_ms) > 0 ? pdMS_TO_TICKS(wakeup_ms) : 1;
    hal_gpio_write(sensor.gpio_pin, 0); // pull wire low
    vTaskDelay(xDelay); // time low phase
    rmt_rx_start(dht_rmt.channel, true); // arm RX channel
    hal_gpio_write(sensor.gpio_pin, 1); // release wire
...
static dht_result_t dht_compute_data22(uint8_t *data, double *temp, double *humi) {
...
dht_result_t read_dht_sensor_data() {
    ...
    err = _dht_read(DHT22_WAKEUP_MS, data);
    ...
    int res = dht_compute_data22(data, &temperature, &humidity);
